 ...
 * @author seba1
 */
public class SikuliFbAutopublisherv2 {
...
"                \"3434679702\"";
 ...
 JOptionPane.showMessageDialog(null,"FINALIZADO CORRECTAMENTE");
      ...
      static public void iniciarPrograma(String url,String grupo,int cantidad) throws InterruptedException, FindFailed, IOException{
...
//abriendo navegador
        ...
        irAUrlYMaximizar(url);
        
        esperarQueEntreFacebook();
        
        buscarBotonCompartir();
   ...
   //en popup     
    if(s.exists(publicar,timeout)!=null){
        ...
        }else if(s.exists(compartirEnTuBiografia)!=null){
    ...
    private static void esperarQueAbraElNavegador() throws InterruptedException {
    ...
    private static void esperarQueEntreFacebook() throws InterruptedException {
       if(s.exists(fbIcon,timeout)==null){
            errorHandler("El link al posteo no se abrio correctamente\n ");
        };
  }

    private static void buscarBotonCompartir() throws InterruptedException {
        s.type(Key.ESC);
int i=0;
    ...
    private static void irAUrlYMaximizar(String url) throws InterruptedException {
             s.paste(url);// ir a url
        s.type(Key.ENTER);
        Thread.sleep(500);
        System.out.println("maximizando navegador...");
        s.type(Key.WIN,Key.UP); //maximizar navegador
    }

    private static void clickBtnCompartirYCompartir() throws InterruptedException, FindFailed{
//chequear compartir y compartir... si no paso se vuelve a clickear compartir
        //compartir
        if(s.exists(btnCompartir)!=null){
          s.click(btnCompartir,timeout);  
        }else{
                    errorHandler("No se visualiza el boton compartir");
                    s.click(btnCompartir,timeout);
        }
   ...
   //compartir...     
        ...
        }else if(s.exists(publicar)==null){
